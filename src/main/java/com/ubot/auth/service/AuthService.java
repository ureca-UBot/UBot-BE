...
public class AuthService {
	...
	public LoginResponseDto loginUser(LoginRequestDto requestDto){
		if (requestDto == null
			...
			throw new UserException(UserErrorCode.INVALID_LOGIN_REQUEST);
		}
	...
	private void validateEmail(String email) {
		...
		if (!EMAIL_PATTERN.matcher(email).matches()) {
			throw new UserException(UserErrorCode.INVALID_EMAIL_FORMAT, "이메일 형식을 맞춰서 입력해주세요. ex) user@naver.com");
		}
	}
	...
	private void validatePassword(String password, String passwordConfirm) {
		if (!StringUtils.hasText(password)
			...
			throw new UserException(UserErrorCode.INVALID_PASSWORD_FORMAT, "비밀번호 혹은 비밀번호 확인 칸이 비었습니다. 입력해주세요.");
		}
		...
		if (passwordByteLength < 8 || passwordByteLength > 20) {
			throw new UserException(UserErrorCode.INVALID_PASSWORD_FORMAT, "비밀번호가 8자 미만이거나 20자 초과입니다.");
		}
