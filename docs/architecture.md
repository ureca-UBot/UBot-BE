...
> 문서 기준 시점: 2026-09-22 (`develop` = `1ac3bf2`, 열린 PR #55·#34 미반영)
...
- **LLM 연동은 아직 구현되지 않았습니다.** 임계값을 넘으면 FAQ 답변을 그대로 반환합니다.
...
| LLM 채팅 | Spring AI Ollama chat (아직 서비스 로직 미연결) | `spring.ai.model.chat: ollama` |
...
- 도메인 테스트: `auth`(통합), `chat`, `store`(컨트롤러·서비스·저장소), `faq`(벡터 저장소), `location`, `embedding`
